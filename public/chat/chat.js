		...
		const generateChatMessage = function (left = true, message) {
			...
			if (message.messageType === 'text') {
				if (message.content.indexOf('[/f0') > -1 || message.content.indexOf('[/g0') > -1) {
					messageContent = `<p class="text-break">${ generateEmojiHtmlMessage(message.content) }</p>`
				} else {
					messageContent = `<p class="text-break">${ message.content }</p>`
				}
				...
				messageContent = `<image style="max-width: 300px;" src="../chat/upload/${ message.content }" />`
			} else if (message.messageType === 'file') {
				let fileName = message.content;
				if (fileName.length > 16) {
					fileName = fileName.slice(0, 12) + '...'
				}
				messageContent = `<button onclick="javascript:window.location='../chat/downloadChatFile?fileName=${ message.content }'" style="border-color: #ebdbdb; width: 210px; height: 60px; background-color: #ebdbdb; padding: 5px 10px; border-radius: 10px; cursor: pointer;" >
